...
| 8 | **Cloud sync for the JSON backup** | Local export/import already works as a flat, versioned JSON envelope (see CHANGELOG) — this is a transport change, not a rewrite. Options: full OAuth to Google Drive/OneDrive (heavy, per-platform); a generic WebDAV/S3 endpoint pointed at the user's own server (light, no vendor lock-in — leaning here); GitHub Gist via a personal access token (zero setup, good bonus alongside WebDAV). Whichever is picked needs to cover `BookLibraryService`/`PodcastLibraryService`/`AnimeLibraryService` too (see CHANGELOG), not just `MyListService`. Lowest urgency — local backup already works. |
