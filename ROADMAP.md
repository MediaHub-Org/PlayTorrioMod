...
| 5 | **Filters for Audiobooks** | Movies/Series/Anime/Music all have real genre filters now (see CHANGELOG). No longer blocked as of 2026-08-27: Open Library's free `search.json` API returns real subject/genre tags from a plain title search, verified against real titles — no auth, no rate limit. Unlike manga's photo-matching risk (a wrong photo is a visibly wrong person), a near-miss genre tag here is cosmetic, not embarrassing — much lower stakes, worth building. Not yet implemented — needs the actual title-matching + filter UI wiring. |
