...
| 9 | **Filters for Music/Radio/Audiobooks**                      | Movies/Series got a decade filter + sort dropdown, and Anime got a genre filter (see CHANGELOG) — both reusing the new shared `FilterDropdown` widget. Music/Radio/Audiobooks don't have equivalent data yet: `MusicTrack` has no genre field, and neither the Radio nor Audiobooks pages have any genre/category concept today. Needs real genre/category data sourced per content type before a filter can be built there — not a UI gap, a data gap. |
| 10 | **Cloud sync for the JSON backup**                         | Local export/import shipped (see CHANGELOG) as a flat, versioned JSON envelope specifically so this is a transport change, not a rewrite — wire it up to a cloud backend (own server, or a drop-in like a user's own cloud storage) instead of only writing to a local file. |
