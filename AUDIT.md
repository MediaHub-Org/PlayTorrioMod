...
| 5 | Manga page has no error handling on load failure → infinite spinner | UX | A network/scrape failure leaves `_isLoading` stuck forever with no message and no retry — looks like the app hung. |
...
| 7 | TLS certificate validation disabled in 5 HTTP clients | Security | `badCertificateCallback = (cert, host, port) => true` accepts any cert for scraped downloads/subtitles — MITM can tamper with downloaded content. |
...
| 9 | `ErrorView` widget exists and is used in Watch hub pages, but not in Manga/Music | UX | Same "consistent UX across sections" principle violation as #4 — the fix already exists in the codebase, it's just not applied everywhere. |
| 10 | Plaintext credential storage (Trakt/Simkl tokens, WebDAV password) | Security | No `flutter_secure_storage`/keychain use anywhere — all in `SharedPreferences`, readable via device/backup access. |
...
- **`lib/services/anime/extractors/tryembed_extractor.dart:57`, `lib/services/books/book_download_service.dart:94`, `lib/services/music/music_download_service.dart:332,354`, `lib/services/subtitles/providers/subtitlecat_provider.dart:16`, `lib/services/subtitles/subtitlecat_service.dart:48`** — **Medium-High**. `HttpClient()..badCertificateCallback = (cert, host, port) => true` unconditionally accepts any TLS certificate for any host. Enables MITM tampering of downloaded book/music/subtitle content and scraped HTML on these code paths.
...
- **`lib/services/trakt/trakt_service.dart:393-394`, `lib/services/simkl/simkl_service.dart:341`** — **Medium**. Trakt/Simkl access & refresh tokens stored in plaintext via `SharedPreferences` (`StorageService.setTraktAccessToken`/`setSimklAccessToken`). No `flutter_secure_storage`/keychain use anywhere in the codebase.
- **`lib/services/backup/cloud_backup_settings.dart:20-49`** — **Medium**. WebDAV username/password stored in plaintext `SharedPreferences` (`cloud_backup_webdav_pass`).
- **`lib/services/backup/backup_service.dart:34-46,100-128`** — **High**. The backup envelope dumps *every* SharedPreferences key — including the plaintext Trakt/Simkl tokens above — into one JSON file/upload. `uploadToCloud`/`downloadFromCloud` use plain `http.put`/`http.get` with no scheme enforcement; if the user enters an `http://` WebDAV URL, all app secrets plus the WebDAV Basic-Auth credentials travel in cleartext. Full-credential exfiltration risk, gated only by how the user types the URL.
...
- **`lib/services/music/music_player_controller.dart:173-293`** (`_loadAndPlayTrack`) — **High**. Creates a new `Player()` (line 207) with no generation guard. If a second call starts before the first's `await MusicService.instance.getAudioStream(...)` (217) resolves — e.g. spamming the skip button — the second call's cleanup (192-200) disposes the *current* `_player`, but the first call's local `player` variable and the listeners it registers afterward (242-273) are untouched by that cleanup. Whichever call reaches `_player = player` (282) last wins; the other's native `Player()` is orphaned (leak), and both tracks can audibly overlap momentarily.
- **`lib/pages/audiobooks/audiobook_player_screen.dart:120-310`** (`_initChapter`) — **Medium-High**. Same shape: new `Player()` per chapter (line 218), no guard against overlapping calls from rapid "next chapter" taps or `player.stream.completed` autoplay firing mid-skip.
- By contrast, `player_screen.dart` and `iptv_player_page.dart` correctly use a single `late final Player` for the widget's lifetime — this bug is specific to the two controllers that (re)create a player per content item.

### Async-after-dispose

- **`lib/pages/player/player_screen.dart:909-965`** (`_loadSubtitle`) — **Medium**. After two unguarded awaits (subtitle download at 924, file read at 937-938), calls `_setSubtitleScale` (950), which does an unconditional `setState` (986-990) with no `mounted` check anywhere in the chain, and none of it is wrapped in try/catch. `_loadSubtitle` runs fire-and-forget from `_fetchInitialSubtitles` (747); if the user backs out of the player while a slow subtitle download is in flight, this throws unhandled after dispose.
- **`player_screen.dart:986-990` (`_setSubtitleScale`) and `1069-1085` (`_applyVolume`)** — **Low-Medium**. Neither checks `mounted` internally, unlike nearly every other `setState` call in the file. Currently only reached from call sites inside `_initStream`'s enclosing try/catch, so today it's accidentally swallowed rather than crashing — but it's the same fragile pattern as above, one call-site change away from becoming visible.

### Boundary bug

- **`player_screen.dart:518-533`** (`_onPlaybackUpdate`, auto-next-episode trigger) — **Low**. `pos.inSeconds >= dur.inSeconds - 15 && pos.inSeconds > 0` has no lower bound on `dur`. Any source reporting under 15 seconds of duration (bad metadata, a short clip, a live stream misclassified as VOD) makes `dur.inSeconds - 15` negative, firing the "Up Next" dialog almost immediately after playback starts.
...
- **Books** (`books_page.dart:376-380`) / **Music** (`music_page.dart:2150,4386`): heart icon in list rows, red `0xFFE50914` (Books) vs. pink `0xFFFF4B72` (Music) — a third color for the same red-heart concept.
...
- **`lib/pages/manga/manga_page.dart:116-142,144-160`** (`_loadInitialData`/`_loadMore`) — **High**. No try/catch around the scrape calls. On a network/scrape failure the exception propagates unhandled, `_isLoading` never resets, and the UI is stuck on `CircularProgressIndicator` (449-455) forever — indistinguishable from a hang.
- **`lib/pages/music/music_page.dart:148-192`** (`_loadMusicData`) — **Medium**. Does catch and correctly reset `_isLoading`, but only `debugPrint`s the error — no user-facing message or retry, indistinguishable from "nothing available."
- **`lib/widgets/common/error_view.dart`** (a shared error+retry widget) is used correctly in `type_catalog_page.dart:179`, `catalog_page.dart:215`, `discover_page.dart:80` — the Watch hub's browse pages handle this right, but **Manga and Music never import it** — **High**, same consistency-principle violation as the favorite button, except the fix already exists in the codebase.
...
- **`lib/widgets/common/universal_play_bar.dart:140-148`** — **Medium**. The central Play/Pause `IconButton` has no `tooltip`, while the Like button above it (127) and Close button below it (152) both do — a screen-reader user gets "Unlike"/"Close" announced but nothing for the main transport control.
