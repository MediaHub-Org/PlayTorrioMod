...
### Comics addon route ruled out, Audiobooks genre filter unblocked — 2026-08-27
...
- **Audiobooks filter (ROADMAP #5)**: previously framed as blocked with "the same title-matching risk flagged for manga" — that comparison undersold the real difference. Verified Open Library's free `search.json` API returns real subject/genre tags from a plain title search (tested against "Project Hail Mary" and "Harry Potter and the Sorcerer's Stone", both returned rich, accurate subject lists), no auth or rate limit. A near-miss title match here just means a slightly-off genre tag — cosmetic — not manga's risk of a wrong *person's photo* being visibly attached to the wrong author. No longer blocked; not yet implemented.
