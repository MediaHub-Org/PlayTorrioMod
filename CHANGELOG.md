...
### Investigate `ContinueWatchingService`/`PlaybackHistoryService` duplication (ROADMAP #2) — 2026-08-27
- **Investigated, kept separate — not simple duplication.** `ContinueWatchingService` (Home page's "Continue Watching" row) carries saved source/quality/addon info for exact resume without rescraping, syncs to Trakt/Simkl, and deliberately purges anything ≥90% watched — it's "pick up where you left off," not a log. `PlaybackHistoryService` (`CollectionPage`'s Watch-hub History tab) is minimal but does **not** purge finished items, and its `getProgress` is also the *only* resume-position fallback when opening the player from any entry point other than the Continue Watching row (e.g. pressing Play fresh from a details page). It also dedupes by episode id, not by show, so multiple episodes of the same show can appear as separate History rows — `ContinueWatchingService` dedupes to one card per show. A real duplicate write exists (both save on the same 5-second timer in `player_screen.dart`), but merging the stores would need to preserve all three behaviors correctly and can't be verified without live-device playback testing across movie/series/anime/torrent resume paths — left as-is rather than risk a regression in a core, frequently-used feature. Left for a future pass with real device access.
