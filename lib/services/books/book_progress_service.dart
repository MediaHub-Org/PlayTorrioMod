...
/// Ported from PlayTorrioV2's `lib/services/book_progress_service.dart`.
/// Tracks which chapter (and scroll position) the user last read for each
/// downloaded book, and where its epub file lives on disk.
class BookProgress {
...
class BookProgressService {
  ...
  static const _prefsKey = 'pt_book_progress_v1';
  ...
  Future<void> delete(String editionId) async {
    final entries = await loadAll();
    for (final entry in entries.where((e) => e.book.editionId == editionId)) {
      try {
