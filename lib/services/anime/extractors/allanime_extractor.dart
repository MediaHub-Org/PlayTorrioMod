...
class AllAnimeExtractor {
  ...
  Future<String?> _resolveShowId(List<String> titleCandidates, String cat) {
    ...
    return _showIdCache.putIfAbsent(key, () async {
      ...
      for (final q in queries) {
        final id = await _searchOne(q, cat);
  ...
  Future<AllAnimeResult?> extractWithProvider({
    ...
    try {
      ...
      final hit = sources.firstWhere(
        (s) => (s['sourceName'] ?? '').toString().toLowerCase() == pLower,
        orElse: () => const <String, dynamic>{},
      );
      if (hit.isEmpty) return null;

      final rawUrl = (hit['sourceUrl'] ?? '').toString().trim();
      if (rawUrl.isEmpty) return null;

      final link = await _resolveSourceLink(rawUrl);
      if (link == null || link.url.isEmpty) return null;

      if (kDebugMode) {
        debugPrint(
            '[AllAnime] OK provider=$provider ep=$episodeNumber cat=$category '
            'url=${link.url.substring(0, link.url.length.clamp(0, 60))}…');
      }
      return AllAnimeResult(
        url: link.url,
        referer: link.referer ?? '$_refr/',
        origin: link.origin ?? _refr,
        provider: provider,
        tracks: const [],
      );
    } catch (e) {
  ...
  Future<({String url, String? referer, String? origin})?> _resolveSourceLink(
      String sourceUrl) async {
    // 1. Direct external video link (e.g. mp4 upload host)
    if (sourceUrl.startsWith('http://') || sourceUrl.startsWith('https://')) {
      // If it's an embed iframe, we don't have a headless runner here; caller
      // must fall back to the InAppWebView flow. Direct media links pass through.
      ...
      // Unknown iframe host — return null so caller falls back to webview
      return null;
    }

    // 2. Encrypted `--{hex}` blob → decrypt XOR 0x38 to get clock path
    String clockPath;
    if (sourceUrl.startsWith('--')) {
      final hex = sourceUrl.substring(2);
      final bytes = <int>[];
      for (var i = 0; i < hex.length - 1; i += 2) {
        final b = int.tryParse(hex.substring(i, i + 2), radix: 16);
        if (b != null) bytes.add(b ^ 0x38);
      }
      clockPath = utf8.decode(bytes, allowMalformed: true);
    } else {
      clockPath = sourceUrl;
    }

    // Rewrite `/apivtwo/clock?…` → `/apivtwo/clock.json?…` (returns JSON)
    if (clockPath.startsWith('/apivtwo/clock?')) {
      clockPath =
          clockPath.replaceFirst('/apivtwo/clock?', '/apivtwo/clock.json?');
    }
    if (!clockPath.startsWith('http')) {
      clockPath = '$_clockHost$clockPath';
    }

    try {
      final req = await _client.getUrl(Uri.parse(clockPath));
      req.headers
        ..set('User-Agent', _agent)
        ..set('Referer', '$_refr/')
        ..set('Accept', 'application/json, text/plain, */*');
      ...
      final links = (j['links'] as List?) ?? const [];
      if (links.isEmpty) return null;

      // Prefer HLS (wixmp), fall back to highest-priority mp4
      Map<String, dynamic>? best;
      for (final raw in links) {
        if (raw is! Map) continue;
        final map = raw.cast<String, dynamic>();
        if (map['hls'] == true ||
            (map['link'] ?? '').toString().contains('.m3u8')) {
          best = map;
          break;
        }
      }
      best ??= (links.first as Map).cast<String, dynamic>();
      final outUrl = (best['link'] ?? '').toString().trim();
      if (outUrl.isEmpty) return null;

      final uri = Uri.tryParse(outUrl);
      final ref = uri == null ? '$_refr/' : '${uri.scheme}://${uri.host}/';
      final org = uri == null ? _refr : '${uri.scheme}://${uri.host}';
      return (url: outUrl, referer: ref, origin: org);
    } catch (e) {
      if (kDebugMode) debugPrint('[AllAnime] clock resolve failed: $e');
      return null;
    }
  }

  /// AES-256-CTR decryption of the `tobeparsed` hex payload.
  ///
  /// Structure:
  ///   bytes[0]      = skipped
  ///   bytes[1..13]  = 12-byte IV prefix
  ///   IV[12..16]    = `00 00 00 02`
  ///   bytes[13..N-16] = ciphertext (last 16 bytes are GCM tag, ignored for CTR)
  String? _decryptTobeparsed(String hexPayload) {
    try {
      final raw = <int>[];
      for (var i = 0; i < hexPayload.length - 1; i += 2) {
        final b = int.tryParse(hexPayload.substring(i, i + 2), radix: 16);
        if (b != null) raw.add(b);
      }
      if (raw.length < 13 + 16 + 1) return null;
      ...
      final ct = Uint8List.sublistView(Uint8List.fromList(raw), 13, 13 + ctLen);
