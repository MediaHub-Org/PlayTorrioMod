  ...
  Stream<StreamSource> scrapeStream({
    ...
    () async {
      try {
        ...
        final providerTasks = _providers.map((provider) async {
          try {
            final uri = Uri.parse(
                '$_apiBase/api/player/sources?${baseParams.toString()}&provider=$provider');
            ...
            for (final item in sourcesList) {
              ...
              final providerId = (item['provider'] ?? provider).toString().toLowerCase();
              final providerName =
