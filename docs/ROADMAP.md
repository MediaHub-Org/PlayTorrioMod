...
| 18 | **"Movies/Series" → "Movies & Series", "Comics/Manga" → "Comics & Manga"** | Requested 2026-09-01. The code still says the slash form (`HubController.currentSections`'s `label: 'Movies/Series'` / `label: 'Comics/Manga'`, `lib/utils/hub_controller.dart`) — only this file's own Navigation-principle table above already drifted to the ampersand form from an earlier uncommitted edit, so doc and code currently disagree. Also asked: propose better *grouped* names for other paired sections if any exist — worth a quick pass over `currentSections`' remaining pairs (Watch's Movies/Series pairing is the only other sub-tab pairing besides Comics/Manga; Audiobooks/Books/Comics/Manga's own three-way split under Read may be worth a look too) before implementing, rather than renaming just these two and stopping. |
| 19 | **Anime's language toggle → a dropdown with flag + language name** | Requested 2026-09-01. Currently `anime_page.dart:586-591`: a binary `IconButton` (globe icon, tooltip only) toggling `_isArabicMode` on/off — not a dropdown, no flag, no visible label of which language is active. Should become a `FilterDropdown` (the same widget already used right next to it for the genre filter, `anime_page.dart:568`) listing each available language with its flag emoji/icon and name. Only two languages exist today (English/original + Arabic, via the separate `anime_arabic` service) — worth checking whether more are planned before over-building the dropdown for a 2-item list. |
| 20 | **Hero carousel image quality/cropping** | Requested 2026-09-01, investigated same day — three different root causes, not one bug: (1) **Movies & Series** (`type_catalog_page.dart:315`, `_buildHeroSlide`): uses `movie.poster` — a portrait TMDB poster — force-fit with `BoxFit.cover` into a landscape hero slot. The `Movie` model has no backdrop field at all, so the hero has no landscape image to reach for; fixing this needs a real backdrop URL added to the catalog data (a TMDB enrichment call, similar to the existing cast-photo enrichment), not a `BoxFit` tweak. (2) **Live TV** (`iptv_hero_carousel.dart:234-238`): already uses a proper `channel.backdropUrl` with `BoxFit.cover` — the code shape looks right, so the reported cropping/low-res is more likely either the source images themselves being low-res, or the crop region disagreeing with `BoxFit.cover`'s centring for these specific images; needs an actual look at the rendered carousel, not a code-only fix. (3) **Anime**'s carousel is reportedly better but possibly oversized — not yet investigated; needs its own look once (1) and (2) are underway. |
| 21 | **Books, Podcast, Radio on the same UX/UI as Anime; Radio shows the wrong content entirely** | Requested 2026-09-01, investigated same day. Music/Podcasts/Radio all live inside `music_page.dart`'s own bespoke `Stack`-based layout (ambient background, custom tab switching) rather than `BrowseScaffold`'s hero+rows shape Anime uses — a deliberate divergence per `sectioned_hub_scaffold.dart`'s own doc comment ("Music opts out... genuinely different shape"), now being asked to reconsider. Books has a separate, older reason it isn't on `BrowseScaffold` already: `BookResult` carries no cover URL at all (see Declined section below) — that blocker needs solving first or Books' hero/rows would render blank. **Radio is a deeper problem than styling**: `music_page.dart:1550`, `_buildRadioView()` is a hardcoded list of 8 genre labels ("Pop Radio", "Rock Mix", etc.) that, on tap, run a genre-flavoured *song search* (`_onGenreTap(query, isRadio: true)`) — there is no actual radio-station data source wired at all, just genre-browsing reskinned to look like stations. Making Radio "only show radio stations" needs a real station directory/API (e.g. something like the Radio Browser API) before any UI work on it means anything. |
| 22 | **Library: drop the heart icon for "Saved", split it into Liked + Playlist** | Requested 2026-09-01. `LibrarySection.saved('Saved', Icons.favorite_rounded)` (`lib/widgets/common/library_sections.dart:17`) uses a heart for a bucket whose own doc comment says it holds "liked, favourited, bookmarked" all merged together — the heart icon overclaims (implies "liked" specifically) for a tab that's broader than that. Asked: a more generic icon for "Saved" itself, and a sub-split inside it into **Liked** and **Playlist** — where "Playlist" means Watchlist in Watch, an actual music playlist in Listen, and (deliberately not built yet) nothing in Read for now. This touches the one shared `LibrarySection` enum all three hubs already standardized on (see its own doc comment on why that unification happened) — the split needs to stay generic enough to keep that one-shape-three-hubs property, not fork back into three different Library shapes to accommodate per-hub Playlist meanings. **Tension with item 24 below, not yet resolved**: this item says Listen's Playlist goes inside Saved; item 24 says "New Playlist cannot be inside Listen Library" — read together, item 24 is likely about where playlist *creation* lives (not inside Library), not a full reversal of putting playlist *browsing* under Saved, but that's a guess — needs a decision before either is built. |
| 23 | **Library: remove History, it duplicates Continue** | Requested 2026-09-01. They are not actually identical in the code — `_buildInProgressTab` reads `ContinueWatchingService.activeItems` (purged once something finishes, no manual remove), `_buildHistoryTab` reads `historyItems` (the full log, finished items included, with a dismiss action) — but both render through the same shared `_progressList` row builder (`collection_page.dart:244-246`), so they look like the same list with different content, which is presumably why they read as duplicates. Removing History is a real, lossy simplification worth being explicit about: today, once something finishes, it drops out of Continue but survives in History; with History gone and no other change, a finished item would just disappear from Library entirely, with no log and no way to re-find or manually clear it. Worth deciding whether that loss is fine as-is, or whether Continue should stop purging finished items (becoming the merged tab) before History is actually deleted. |
| 24 | **"New Playlist" cannot be inside Listen's Library** | Requested 2026-09-01. Read together with item 22, which says Listen's Playlist grouping belongs inside Saved — see the tension noted there. Needs clarification on scope: does this mean playlist *creation* (a "New Playlist" action/flow) must live outside Library while existing playlists can still be *browsed* under Saved, or that Playlist as a concept shouldn't be under Library for Listen at all? |
| 25 | **Audiobook details page: fix a genuine content duplication, and general UI polish** | Requested 2026-09-01, investigated same day — real bug, not a matter of taste. `audiobook_detail_page.dart`'s outer `build()` (~lines 191-301) already renders the cover (as a Hero-tagged image), the title and a `_PlayFirstChapterButton`; that same `build()` then also calls `_buildDetails()` (~lines 333/341, calling into 459-500) which renders its *own* cover image, title and another `_PlayFirstChapterButton` — the same three pieces of content appear to render twice on the same page. Also uses the ad-hoc `screenW < 700` breakpoint check (item 6's pattern) instead of `AppBreakpoints.of(context)`, worth fixing in the same pass since the file is already being touched. "Fix the UI" beyond the duplication is unscoped — needs a look at the actual rendered page (not just the code) to know what else is wrong. |
| 26 | **Opening a Details page blocks the hub pills/bottom bar from doing anything visible** | Requested 2026-09-01, root-caused same day. Each hub's content lives in its own `NestedNavigator` (`hub_page.dart`), kept alive in an `IndexedStack` so switching hubs doesn't rebuild them. A pushed Details page lives on that *same* nested Navigator's stack, on top of the section content. Tapping a different hub pill or bottom-bar section only changes `HubController`'s state (`currentHub`/`currentSectionId`) — nothing pops the nested Navigator back to its root first, so a Details page pushed on top keeps covering the screen and the section switch underneath is invisible until the user manually backs out. There's already a partial, currently-dead piece of infrastructure for exactly this shape of problem — `HubNavigator.goHome()` (`lib/utils/hub_navigator.dart`) — registered by `HubPage` but never called from anywhere; extending that pattern (pop each hub's nested Navigator to root when its hub/section changes) is the likely fix shape, not a new mechanism from scratch. |
