...
| 7b | **The Music Player Studio's "Mini Player" tab configures a widget that does not exist**            | This fork replaced upstream's per-page mini players with the shared `UniversalPlayBar`, which reads no `MusicSettings`. So the studio's preset picker, drag-to-reorder and live preview all persist state (`selectedMiniPreset`, `componentOrderMini`) that nothing consumes — a settings screen that lies. Either remove the Mini Player half of the studio (consistent with one-shared-bar, loses nothing that works today) or teach `UniversalPlayBar` to honour it, which first needs an answer for what a *music* preset means when the bar is showing a video or an audiobook. Found 2026-08-31. |
| 8  | **`actions/download-artifact` is still on v4**  | Checked 2026-08-31 and there is **nothing to fix**, recorded so it is not re-raised. The only documented incompatibility is that v4-and-above cannot read artifacts from `upload-artifact@v3` or below, so v4 download alongside v5 upload is a supported pair. Nor is it a runtime issue: `upload-artifact@v5`, `download-artifact@v5` and `download-artifact@v6` all declare `using: node20` in their own `action.yml`, so none of the v5 bumps moved anything off Node 20. Bumping it would be churn. Revisit only if a release run actually warns. |
