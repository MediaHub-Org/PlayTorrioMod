...
| 16 | **Rename the three hub names (Watch/Listen/Read)** | Requested 2026-08-31. **New names not yet chosen** — needs a decision from the user before this can be implemented; ROADMAP entry exists only to track the request, not scope it. `AppHub.navLabel` (`lib/utils/app_hub.dart`) is the single source of truth for the three labels, so once names are picked this is a small, low-risk change — but every other doc/string that spells out "Watch, Listen, Read" (`about_settings_page.dart`'s `_HubList`, this file's own Navigation principle table, `README.md`, `FORK_DIFFERENCES.md`) needs updating alongside it or they go stale the same way PlayTorrioMov's copied docs did. |
| 17 | **Move the Settings icon, "on top"** | Requested 2026-08-31. Exact target position not yet clear from the request — needs a quick clarification (top of which chrome, and relative to what: the hub pills, the wordmark, per breakpoint?) before touching `TopBar`/`AdaptiveNavShell`. |
| 18 | **Opening Settings must not hide the three hub categories, on any breakpoint** | Requested 2026-08-31. Today it does: `HubPage` opens Settings via `Navigator.push(..., LiquidRevealRoute(page: const SettingsPage()))`, a full-screen route that covers the persistent `TopBar`/hub-pills and the mobile bottom bar entirely — true on mobile, tablet, desktop and (untested) TV alike, since it's the same push regardless of `AppBreakpoints.of(context)`. Making Settings coexist with the nav chrome instead (an overlay panel, a split view, a sheet that leaves the hub switcher reachable) is a real architecture change to `HubPage`'s settings entry point and possibly `SettingsPage`'s own `Scaffold`, not a one-line fix — needs its own design pass across all four breakpoints before implementation, TV especially since D-pad focus would need to move between the panel and the nav chrome. |
