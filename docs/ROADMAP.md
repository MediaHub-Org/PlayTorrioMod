...
| 17 | **Opening Settings must not hide the three hub categories or their sections, on any breakpoint** | Requested 2026-08-31, expanded same day: covers both nav layers, not just the hub switcher — the hub pills/tabs (Watch/Listen/Read) *and* each hub's own section row (e.g. Watch's Movies/Series · Anime · Live TV · Library) must both stay visible/reachable while Settings is open. Today neither survives: `HubPage` opens Settings via `Navigator.push(..., LiquidRevealRoute(page: const SettingsPage()))`, a full-screen route that covers the persistent `TopBar` (hub pills) and, per hub, `SectionTopBar`'s chip row on tablet/desktop or the equivalent bottom bar row on mobile — true on mobile, tablet, desktop and (untested) TV alike, since it's the same push regardless of `AppBreakpoints.of(context)`. On mobile specifically, the section row already lives in the bottom bar (see `AdaptiveNavShell`'s `_MobileSectionTabBar`) — the ask is that this survive Settings opening too, not that it move there (it's already there). Making Settings coexist with both nav layers is a real architecture change to `HubPage`'s settings entry point and possibly `SettingsPage`'s own `Scaffold`, not a one-line fix — needs its own design pass across all four breakpoints before implementation, TV especially since D-pad focus would need to move between the panel and two layers of nav chrome. Item 16 (settings icon position) lands together with this. |
