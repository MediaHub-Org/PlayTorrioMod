...
### Music header margin, Podcasts/Audiobooks live-switch bug, album/playlist play button consistency, ROADMAP #12 live-verified (still open) — 2026-09-02
- **#12 live-tested with a real close-while-playing-video: reproduced.** Launched from a fresh terminal with full logging, played a stream at 2160p, closed the window. System Event Viewer logged the exact `PlayTorrioMod.exe - System Error : Unknown Hard Error` popup at the moment of close, even though the app's own log confirmed `VideoOutput::~VideoOutput` ran (the `onShutdownDispose` candidate fix from earlier today did fire). A control run with no active playback closed clean, zero Event Viewer entries — still specifically an active-stream-at-close issue, just not the one the candidate fix targeted. See `ROADMAP.md` #12 for the full writeup; reopened, root cause still unknown.
- Music tab's title/genre/search row now uses 24px margin, matching Radio and Podcasts & Audiobooks in the same column position (was 8px).
- Fixed Podcasts ↔ Audiobooks sub-tab not switching live: `HubController.spokenAudioType` changes only triggered a rebuild when the top-level Music/Radio/Podcasts/Library tab also changed, so toggling the sub-tab while already on that section silently did nothing until something else forced a rebuild (matches the report: "changes only when you change the page and came back").
- Album and playlist cover cards' play buttons are now consistent: both hover-only (never shown by default, even while playing), a small bottom-right icon instead of a full-cover center overlay, and both toggle play/pause correctly. Added `MusicPlayerController.currentQueueSourceId` so playlists can show a real playing state the same way albums already did, instead of a plain always-play icon.
...
- **Radio gains real search and liking**, neither existed before. `RadioBrowserService.search` (name search via the same free radio-browser.info API already used for genre browsing) behind its own header icon, replacing reliance on the now-fixed wrong shared one. New `RadioLibraryService` (mirrors `PodcastLibraryService`) plus a heart on each station card — no details page to put it on, unlike Podcasts/Audiobooks — and Radio joins Listen Library > Saved > Liked's type filter chips (Music/Podcasts/Audiobooks/Radio).
- **Album and playlist covers gain a real play button.** Tapping the cover already opened the detail page correctly (not a regression); what was missing was any way to play *from* the card. Added a hover-fade-in play/pause button (`_MusicCoverCard`, self-contained via `MusicPlayerController.instance`/`MusicService.instance` rather than threading a callback through every row/grid that builds one). Albums show a real playing/paused state via `MusicTrack.albumId` matching; playlists show a plain play icon since there's no reverse link from a track back to which curated playlist it came from.
- **Music gains a genre filter next to search**, reusing the `_selectGenre`/`_backToGenres` flow the existing genre-tile slider already drove — no new state.
- **Manga reordered**: a static "Manga" title (was a dynamic "Discover Manga"/genre-name heading), genre pills + customize + search sharing one row (was two separate rows), Continue Reading moved to after that row instead of before it.
- **Settings icon no longer shifts position between mobile and desktop.** Each had its own hand-tuned `IconButton` — different size/padding put the icon a few pixels off between the two. Extracted one shared `SettingsIconButton` (`top_bar.dart`), defined mobile-first (the tighter `36x36`/zero-padding constraint fits both cases; desktop's roomier default didn't need to be the baseline).
- **Checked, not our bug**: "Anime fails to load" — AniList's own API is down ("temporarily disabled due to severe stability issues", confirmed via a direct query). The app already shows a graceful error view instead of crashing.
- **Fixed podcast playback not starting.** `PodcastPlayerController` was still on `video_player` (an audio-only workaround), the same silent-failure class already fixed for Audiobooks earlier this session — a stream that failed to initialize was swallowed into a `debugPrint`-only catch, so tapping an episode looked like nothing happened. Migrated to `media_kit`'s `Player`, mirroring `AudiobookPlayerController`'s proven pattern (open-paused-then-play, `stream.playing`/`position`/`duration`/`error` subscriptions, cancel-and-dispose cleanup).

### Video/IPTV player never disposed on window close — candidate fix for the "Unknown hard error" (ROADMAP #12) — 2026-09-02
- A native Windows window close never walks the widget tree calling `State.dispose()` — `WindowService._shutdownAndClose()` is the only code that runs before the window actually goes down. It called `PlaybackCoordinator.stopActive()`, which only pauses a source unless it registered `onFullStop`. Music and Audiobooks both do (their `stop()` disposes their `media_kit` `Player`); `PlayerScreen` (movies/series/anime) and `IptvPlayerPage` never did — their real cleanup lived only in their own `dispose()`, which a window close never reaches. Their native libmpv decoder threads and GPU surface stayed alive into process teardown.
- Timing lines up with when #12 was reported: a *different* access-violation fix (`cf97c81`, 2026-08-28) resolved `window_manager`'s own `destroy()` bypassing the engine's `WM_CLOSE` chain, verified with zero active playback; the `media_kit` engine swap merged 20 minutes later the same day; #12 surfaced 3 days after both — matching the ticket's own "only after specific actions like an active stream" guess.
- Added `PlaybackCoordinator.disposeForShutdown()` and a new `activate()` param, `onShutdownDispose`, kept distinct from `onFullStop`: unconditional dispose is only safe when nothing runs afterward (shutdown) — during normal use a source's widget may still be mounted after a Stop tap and must stay in a safe state, which `onFullStop` already accounts for. Defaults to `onFullStop` when a source doesn't set one explicitly, so Music/Audiobooks needed no changes. Wired `PlayerScreen` and `IptvPlayerPage` to it; `WindowService` now calls `disposeForShutdown()` instead of `stopActive()`.
- Verified: analyzer clean, full suite green, and a real `WM_CLOSE` (via `Process.CloseMainWindow()`, not a force-kill) exits clean with zero new Event Viewer entries. **Not yet verified with an actively-playing video at close time** — no way to drive the app's UI to start real playback in this environment. Root cause is well-evidenced from code, not empirically reproduced-then-fixed; ROADMAP #12 stays open pending a hands-on check.

### Movies & Series reaches parity with Anime/upstream's Home; nav restructure; consistent search UX (ROADMAP #13) — 2026-09-02
- **Movies & Series' hero now matches Anime's**: rating badge, genre chips, a Watch Now / Details button row (was a whole-slide tap-to-open gesture, which Anime's hero doesn't have either). `DetailsPage` gains an `autoPlay` param so Watch Now jumps straight to playback once details load.
- **Hero fills to the top of the page, header floats and stays visible while scrolling** — `BrowseScaffold` gained an opt-in `overlayHeader` flag (the header renders as a `Positioned` overlay instead of a flow child), plus a desktop-only `CustomScrollTrack` (Anime's own scroll indicator, absent before). `BrowseScaffold` has exactly one consumer (this page), so this carried no risk elsewhere.
- **Hero image was showing the wrong picture for some addons.** Two rounds: first found the catalog list's own `movie.background` field carries no guarantee of actually being a landscape image (unlike Anime's AniList `bannerImage`, which is curated) — one addon put an unrelated portrait image there. Checked upstream's own Home hero (`home_page.dart:1000`) for the reference behavior: `detail?.background ?? movie.poster`, where `detail` is the *per-item enriched* `MetadataService.fetchMeta()` result this page already fetches for genre/synopsis (`_heroDetails`) — not the raw catalog-list field. Matches upstream exactly now.
- **Continue Watching, Latest Releases, a Series-only Calendar row, and the Movies/Series pill switcher moved into the header** — the browse-template plan (`docs/superpowers/plans/2026-09-02-browse-template.md`) shipped in full: `PillTabRow` extracted from `SectionSubTabs`, `BrowseScaffold` gained `belowHero`/`afterRows` slots, `ContinueWatchingSlider` gained real `movie`/`series` filter values, `UpcomingCalendarRow` added, `latestReleases()` added.
- **Grid covers stopped growing unbounded on wide desktop windows** — both this page's filtered grid and Saved shared the same `crossAxisCount` breakpoint table, capped at 6 columns past 1200px. Added a 7-column tier past 1600px in both places.
- **Watchlist/Watched/Liked buttons on the details page were crowding each other** — gaps widened, `LikeButton`'s icon-style padding matched to the other two buttons' visual weight.

### Nav restructure: Audiobooks joins Listen (paired with Podcasts), Comics/Manga split in Read — 2026-09-02
- Read hub: Audiobooks removed; Comics and Manga (previously one "Comics & Manga" section behind a sub-tab) are now independent top-level sections — **Books · Comics · Manga · Library**, backfilling the slot Audiobooks leaving frees up.
- Listen hub: Podcasts' section now holds Podcasts and Audiobooks behind a sub-tab (`HubController.spokenAudioType`) — **Music · Podcasts/Audiobooks · Radio · Library**. Both are episodic spoken audio, the same reasoning Watch already uses for Movies/Series sharing one section.
- Both hubs stay at exactly four sections, per the Navigation principle.
- Listen/Read Library's Saved tab redone to match Watch's own `CollectionPage`: type is a filter chip inside one grid (`_LikedEntry`/`_ReadLikedEntry`, merging each hub's separately-persisted liked stores for display only), not a sub-page per content type — an earlier pass in this same session had built it as sub-tabs first, corrected after review. Neither hub gained a Watched/Watchlist-equivalent: Listen doesn't fit that model, and Read has no "finished" flag to filter by (would be new tracking, left out of scope).
- Listen's Library gains Downloads/In Progress/Saved coverage for Audiobooks (previously Read's Library); Read's own Downloads tab, which was audiobook chapters exclusively, now shows an honest empty state.
- `AudiobookPlayerController` (the background/mini-bar controller) moved from `video_player` (an audio-only video player, a leftover workaround) to `media_kit`/libmpv — the same engine `MusicPlayerController` and the fullscreen `AudiobookPlayerScreen` already used. Public API unchanged, so no UI code needed to change.

### Consistent search UX for Podcasts/Books/Audiobooks; Audiobooks' search was actually broken — 2026-09-02
- The shared `PageSearchButton` navigates to `SearchPage`, which is hardwired to `AddonManager.searchAll` (the Stremio addon catalog) — Podcasts (iTunes API), Books (libgen.li) and Audiobooks (AudiobookBay) each need their own backend, so that route was never functional for them. **Audiobooks had no working free-text search at all** — the button pointed at the dead addon route, and the page silently defaulted to a hardcoded `_performSearch('Harry Potter')` with category pills as the only real input.
- Added `SimpleSearchPage<T>`, a generic full-screen search page (back button + field in the header, list or grid results) matching `PageSearchButton`/`SearchPage`'s own shape. Wired Podcasts, Books and Audiobooks to it against their real services, removing each page's bespoke inline search state.
- Podcasts gained a real welcome/browse page (Apple's public Top Podcasts chart) instead of a bare search box before any query — Books already had one (`_browsing`/`browseRecent()`), just not visible in the screenshot that first raised this.
- Books gained Language/Format filter chips — the only fields libgen.li's list view actually exposes (no genre/subject, no cover art — `ROADMAP.md`'s Declined section already covers why a grid/`BrowseScaffold` layout isn't viable there).

### TorrServer fails to start after an unclean previous session — 2026-09-02
- A previous session that didn't shut down cleanly (crash, force-kill, a hot-run detach) could leave `torrserver.exe` running standalone, still holding bboltDB's single-writer lock on `config.db` — the next `start()` then failed with a DNS-check timeout followed by `Error open bboltDB`. `torrserver_flutter` is a pub.dev-hosted package, not code in this repo, so the fix lives in `TorrentStreamService.start()` instead: Windows-only, best-effort — check `tasklist` for an already-running `torrserver.exe` before starting (only reached when this session's own controller believes nothing is running, so any hit is necessarily stale) and `taskkill` it. Verified live: force-killed only the app while its spawned `torrserver.exe` survived, confirmed the orphan, then watched a fresh launch detect and kill it before a clean start.
...
- **Declined**: folding `AnimeLibraryService`'s four-state tracking into `MyListService`'s three-flag model (see ROADMAP's Declined section for the reasoning) — Simkl-synced anime already shows correctly under Saved's Anime chip, which covers the case that actually reaches a shared list.

### Watched now syncs to Trakt/Simkl; VideoPlayerSettingsPage reachable again; music seekbar default corrected, fullscreen play/pause icon fixed (ROADMAP #17/#24/#26) — 2026-09-02
- **Watched syncs to the cloud.** `MyListService.setWatched`/un-watching now call `TraktService.addToHistory`/`removeFromHistory` (`/sync/history`) and `SimklService.markWatched`/`markUnwatched` when authenticated — both endpoints already existed in this codebase, unused for this purpose. Mirrors how `setWatchlist` already synced. Liked stays local-only, as decided for item 25.
- **`VideoPlayerSettingsPage` (this fork's decoder/buffer/subtitle settings) is reachable again** — added a "Playback & Decoder" tile in Settings, right beside the Anime4K one that landed in the same upstream merge. It had zero navigation call sites anywhere in the app; the settings themselves were always working, there was just no way to reach the screen.
- **Corrected an earlier claim of mine**: item 24 said the music seekbar had no drag-to-scrub support at all. Rechecked — every style variant already has full `onHorizontalDragUpdate` handling, including a `standardSlider` style backed by a real Flutter `Slider`. The default was just set to the fancier animated `waveformEqualizer` style; changed the default to `standardSlider` to match the user's own stated preference for something simpler.
- **Fullscreen music player's play/pause icon actually was stale, but not for the reason first guessed.** `_MusicExpandedPlayer` is pushed as its own route, not a child of `MusicPage`'s widget tree — `MusicPage` already listening to `MusicPlayerController` never rebuilt it. Now listens to the controller directly and disposes the listener on close, so play/pause (and the vinyl spin animation) reflect live state instead of whatever they were on first build.
...
- **#22 follow-up, 2026-09-02**: the Saved tab itself still used `Icons.favorite_rounded` — missed in the first pass, which only added the Liked/Watchlist chips inside it. Swapped for `Icons.inventory_2_rounded`; the heart now lives only on the Liked chip, where it's an accurate claim.
